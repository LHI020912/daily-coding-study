<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    ...
    <head>
        ...
        <script type='text/javascript'>
        	...
        	// 리터럴 표기법은 정규식 패턴 문자를 / 정규식패턴문자 / 로 묶어서 인스턴스 생성하는 기능
        	let re1 = new RegExp('ab+c'); // 정규식에서 +는 반복을 의미, +앞의 문자를 1회 이상 반복하는 패턴
        	let re2 = new RegExp(/abc/); // 문자열에 abc 패턴 있는지 확인
        	let re3 = /abc/;
        	
        	// RegExp.test("문자열") : 파라미터로 전달된 문자열이 정규표현식의 패턴과 일치하는지의 결과 ture/false로 반환하는 메소드
        	...
        	/*  정규식 기호: ^(시작값을 의미, 먼저 표시), +(1회 이상 반복), $(종료값을 의미, 뒤에표시)
        		정규식 기호: [] (패턴의 집합), |(or의 의미), - (순서적인 범위를 표현)
        		[]안의 문자중 하나로 시작해야되고 문자는 1번이상 반복될 수 있고 []안의 문자중 하나로 끝나야 함
        	*/
        	const regex = /^[ㄱ-ㅎ|가-힣|a-z|A-Z|0-9| |]+$ /;
        	console.log(regex.test("<script"));	// <로 시작
        	console.log(regex.test("script>"));	// >로 끝
        	console.log(regex.test("scr<ipt"));	// []안에 없는 문자가 포함
        	console.log(regex.test("script"));	// []안에있는 문자로 시작되고 종료되는 반복
    ...
    <%  %>
        ...
        <div class="container">
            ...
            <% 
            	...
            	if(key == null) { key = ""; }
            %>
             <div class="container__header"> 검색어 <%= key  %>
             </div>
        </div>
    </body>
