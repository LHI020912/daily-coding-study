...
public class StudentInjectMain {
	...
	public static void main(String[] args) {
		...
		// java.sql 패키지의 모든 객체는 오류를 던지기 때문에 사용측에서는 예외처리가 반드시 필용
		try {
			System.out.print("학생 번호 입력: ");
			String stdNo = sc.nextLine();
			System.out.println(stdNo);
			
			// select 쿼리문 작성: 입력도니 학생 번호에 대한 정보를 출력
			// 질의어 ? 없이 직접작성
			// 문제점 : preparedStatment 사용했지만 일반쿼리 이용 -? 홀더 사용하지 않음
			 String sql = "select * from student where stdNo = '"+ stdNo +"'";
			// 입력한 모든 값은 DB에 일반값으로 전달됨(질의 문자열로 전달되지 않음)
