...
public class MemberLoginKeySalt {
	...
	public static String md5(String pass) throws Exception {
		...
		try {
			...
			byte[] digest = md.digest(); // update된 데이터를 salt타입의 해시값으로 변경해서 반환
			System.out.println("key-Salt 타입 해시값: "+digest);
			
			// salt 타입의 해시값을 문자열로 변환
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			if(rs.next()) {
				...
				String hashDb = MemberjoinKeySalt.Hashing(memPass.getBytes(),dbSalt);
				System.out.println(dbPwd);
				System.out.println(dbSalt);
				if(dbPwd.equals(hashDb)) {
