...
<body>
	...
	title1를 대문자로 변경 (fn:toUpperCase()) : ${fn:toUpperCase(title1)}<br>
	title1를 소문자로 변경 (fn:toLowerCase()) : ${fn:toLowerCase(tutle2)}<br>
	
	3번째 ~ 5번째 문자열 추출 (fn:substring()) : ${fn:substring(title1,2,5)}<br>
	앞뒤 공백 제거 (fn:trim()) : ${fn:trim(title1)}<br>
	공백을 /로 변환 (fn:replace()) : ${fn:replace("title1","","/")}<br>
	
	title1에 '중심' 포함여부 확인 (fn:contains()) : ${fn:contains(title1,str1)}<br>
	title2에 '중심' 포함여부 확인 (fn:contains()) : ${fn:contains(title2,str1)}<br>
